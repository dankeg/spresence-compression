...
int icer_compress_image_yuv_uint8(uint8_t *y_channel, uint8_t *u_channel, uint8_t *v_channel, size_t image_w,
    ...
    // Support flash streaming during rearrange
    // If flash write callback is provided, write directly to flash instead of RAM
    int use_flash = (output_data->rearrange_flash_write != NULL);
                    ...
                    for (int chan = ICER_CHANNEL_MIN;chan <= ICER_CHANNEL_MAX;chan++) {
                        if (icer_rearrange_segments_8[chan][i][j][lsb][k] != NULL) {
                            ...
                            if (use_flash) {
                                ...
                                size_t written = output_data->rearrange_flash_write(
                                    output_data->rearrange_flash_context,
                                    icer_rearrange_segments_8[chan][i][j][lsb][k],
                                    len
                                );
                                if (written != len) {
                                    // Flash write failed - this is a critical error
                                    return ICER_FATAL_ERROR;
                                }
    ...
    output_data->size_used = rearrange_offset;
    if (use_flash) {
        output_data->rearrange_flash_offset = rearrange_offset;
    }
...
int icer_compress_image_yuv_uint16(uint16_t *y_channel, uint16_t *u_channel, uint16_t *v_channel, size_t image_w,
    ...
    // Skip wavelet transform if channels are already transformed
    // Check if output_data has a flag indicating channels are pre-transformed
    bool skip_wavelet = (output_data->channels_pre_transformed != 0);
