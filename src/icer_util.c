...
int icer_init_output_struct(icer_output_data_buf_typedef *out, uint8_t *data, size_t buf_len, size_t byte_quota) {
    // If flash write callback is already set, allow smaller buffer
    // Otherwise, require full buffer for RAM-based rearrange
    if (out->rearrange_flash_write == NULL) {
        // RAM-based: require full buffer
    if (byte_quota * 2 > buf_len) return ICER_OUTPUT_BUF_TOO_SMALL;
    } else {
        // Flash-based: only need data_start region
        if (byte_quota > buf_len) return ICER_OUTPUT_BUF_TOO_SMALL;
    }
    ...
    if (out->rearrange_flash_write == NULL) {
        // RAM-based: set rearrange_start pointer
    out->rearrange_start = data + byte_quota;
    } else {
        // Flash-based: rearrange_start not used (data goes to flash)
        out->rearrange_start = NULL;
    }
    
    // Initialize flash streaming fields (if not already set)
    if (out->rearrange_flash_write == NULL) {
        out->rearrange_flash_write = NULL;
        out->rearrange_flash_context = NULL;
        out->rearrange_flash_offset = 0;
    }
    
    // Always initialize pre-transformed flag
    // (This flag is independent of flash streaming, so initialize it unconditionally)
    out->channels_pre_transformed = 0;
